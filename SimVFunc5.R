...
# this script is (5) validation data +1 (only if  0 cell in validation data)
...
# =================================================================================================
# This is scenario 5:  +1 continuity correction
set.seed(789)
...
SimVFunc <- function(E, R, RR, Se, Sp, n, num.rep, num.subiter){
  ...
  for (k in seq_along(num.rep)) {
    for (h in seq_along(num.subiter)){
        ...
        ## for cases
        # validation data +1 (only if  0 cell in validation data)
        ppvD1Alpha <- ifelse(misD1E1==sample, D1_E1E1, mapply(rbinom,1,sample, D1_E1E1/(D1_E1E1+D1_E0E1)))
        ppvD1Beta <- sample - ppvD1Alpha
        ppvD1Alpha_new <- ifelse(ppvD1Alpha == 0 | ppvD1Beta == 0,ppvD1Alpha + 1, ppvD1Alpha)
        ppvD1Beta_new <- ifelse(ppvD1Alpha == 0 | ppvD1Beta == 0,ppvD1Beta + 1, ppvD1Beta)
        
        npvD1Beta <- ifelse(misD1E0==sample, D1_E1E0, mapply(rbinom,1,sample, D1_E1E0/(D1_E1E0+D1_E0E0)))
        npvD1Alpha <- sample - npvD1Beta
        npvD1Alpha_new <- ifelse(npvD1Alpha == 0 | npvD1Beta == 0,npvD1Alpha + 1, npvD1Alpha)
        npvD1Beta_new <- ifelse(npvD1Alpha == 0 | npvD1Beta == 0,npvD1Beta + 1, npvD1Beta)
        
        ## for non-cases
        # validation data +1 (only if  0 cell in validation data)
        ppvD0Alpha <- ifelse(misD0E1==sample, D0_E1E1, mapply(rbinom,1,sample, D0_E1E1/(D0_E1E1+D0_E0E1)))
        ppvD0Beta <- sample - ppvD0Alpha
        ppvD0Alpha_new <- ifelse(ppvD0Alpha == 0 | ppvD0Beta == 0,ppvD0Alpha + 1, ppvD0Alpha)
        ppvD0Beta_new <- ifelse(ppvD0Alpha == 0 | ppvD0Beta == 0,ppvD0Beta + 1, ppvD0Beta)
        
        npvD0Beta <- ifelse(misD0E0==sample, D0_E1E0, mapply(rbinom,1,sample, D0_E1E0/(D0_E1E0+D0_E0E0)))
        npvD0Alpha <- sample - npvD0Beta
        npvD0Alpha_new <- ifelse(npvD0Alpha == 0 | npvD0Beta == 0,npvD0Alpha + 1, npvD0Alpha)
        npvD0Beta_new <- ifelse(npvD0Alpha == 0 | npvD0Beta == 0,npvD0Beta + 1, npvD0Beta)
...
write.xlsx(final[[1]], file = paste0("S5_result100k_", Sys.Date(), ".xlsx"))
write.xlsx(final[[2]], file = paste0("S5_Data100k_", Sys.Date(), ".xlsx"))
# population size 10k
final2 <- SimVFunc(E = 0.2, R = 0.01, RR = 2, Se = 0.6,  Sp = 0.98, n = 10000, num.rep = 100000, num.subiter =10000)
write.xlsx(final2[[1]], file = paste0("S5_result10k_", Sys.Date(), ".xlsx"))
write.xlsx(final2[[2]], file = paste0("S5_Data10k_", Sys.Date(), ".xlsx"))
